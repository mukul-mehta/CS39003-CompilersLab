...
int readf(float *fP)
{
	...
	while (1)
	{
		...
		else
		{
			...
			{
				if (!IsFraction && status == OK)
					val = 10 * val + (int)(str[0] - '0');
				else if (IsFraction && Dot == 1 && status == OK && (!first || (first && str[0] != '0')))
				{
					fraction = 10 * fraction + (int)(str[0] - '0');
					frac_cnt++;
				}
				else if (IsFraction && Dot == 1 && first && str[0] == '0' && status == OK)
					frac_cnt++;
				else
					status = ERR;
				first = 0;
